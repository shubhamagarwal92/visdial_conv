...
class SubsetComplementVisDial:
    ...
    def extract_ndcg(self, model_type: str,
        ...
        indices_list = random.sample(range(len(ranks_phase1_json)),k=num_samples)
        ...
        assert len(ranks_phase1_json) == len(ranks_finetune_json) == len(new_annotation_list)
        gt_relevance_list = []
        ranks_list_phase1 = []
        ranks_list_finetune = []
        ndcg_list_phase1 = []
        ...
        for indx in range(len(ranks_phase1_json)):
            ...
            assert ranks_phase1_json[indx]['round_id'] == new_annotation_list[indx]['round_id']
            assert ranks_finetune_json[indx]['round_id'] == new_annotation_list[indx]['round_id']
        ...
        ndcg_write_path = self._get_ndcg_path(self.model_preds_root, model_type,
                                              phase="sparse",
        ...
        ndcg_write_path = self._get_ndcg_path(self.model_preds_root, model_type,
                                              phase="finetune", subset_type=self.subset_type)
        print(f"Saving as {ndcg_write_path}")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="data/crowdsourced/visdial_1.0_val_dense_annotations.json",
        help="Path to data directory with json dialogues."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-s", "--subset_type", default="_complement",
        help="Subset type."
    )
...
if __name__ == "__main__":
    config = parse_args()
    ranker = SubsetComplementVisDial(config)
    ...
    for model_type in model_folder_list:
        ranker.extract_ndcg(model_type, num_samples=97)

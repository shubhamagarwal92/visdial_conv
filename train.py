...
def train_dense(config, args, dataloader_dic,
          device, finetune: bool = False, load_pthpath: str = ""):
    ## Training on new dense annotations that are formed
    # =============================================================================
    #   SETUP BEFORE TRAINING LOOP
    # =============================================================================
    train_dataset = dataloader_dic["train_dataset"]
    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        for i, batch in enumerate(tqdm(combined_dataloader)):
            ...
            target = batch["augmented_gt_relevance"]
            # Same as for ndcg validation, only one round is present
            # output = output[
            #         torch.arange(output.size(0)), batch["round_id"] - 1, :
            #     ]
            batch_size, num_rounds, num_options = output.size()
            output = output.view(batch_size * num_rounds, num_options)
            target = target.view(batch_size * num_rounds, num_options)
...
def main():
    ...
    # Training on augmented dense annotations
    if args.phase in ["dense"]:
        print("Starting training on dense annotations")
        # SA: todo checkpoint path
        dataloader_dic = get_dataloader(config, args, finetune=False, use_augment_dense=True)
        train_dense(config, args, dataloader_dic, device, finetune=False, load_pthpath=args.load_pthpath)
